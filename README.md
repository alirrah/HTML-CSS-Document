...
معمولا اسم فایل css پروژه مان را style.css می زاریم.
...
برای دادن رنگ پس زمینه در CSS از خصیصه backgrand-color استفاده می کنیم.
...
فرم rgb(NUMBER, NUMBER, NUMBER) بدین صورت است که شامل 3 عدد بین 0 تا 255 می شود و به ترتیب مقدار قرمز، سبز و آبی بودن را مشخص می کند.

برای انتخاب رنگ می توانید [flat color](https://www.google.com/search?q=flat+color) و [material ui](https://www.google.com/search?q=material+ui) را سرچ کنید.
...
یادتان نره معیار اندازه گیری را مشخص کنید. فعلا فقط px برای پیکسل می دانیم.
...
با خصیصه text-align برای محل قرار گرقتن متن است که مقادیر آن مeft, right, center, justify است که پیش فرض آن left است.

با خصیصه font-size اندازه و سایز فونت را تعیین می کنیم. مشخص کردن معیار اندازه گیری را فراموش نکنید.

اگر متن فارسی و انگلیسی باشد ترتیب به هم می خورد پس بهش direction می دهیم. که برای فارسی مقدارش را برابر rtl و برای انگلیسی که به صورت پیش فرض نیز هست ltr قرار می دهیم.
...
برای اینکه در سایت های فارسی متن فارسی و انگلیسی درست باشد به تگ body این خصیصه را می دهند تا مشکلی پیش نیاید.
...
مقدار none برای پاک کردن خط زیر لینک ها کاربرد دارد.

بعضی از این کار ها را هم با css می شود اعمال کرد هم با تگ ها ولی فرق اینا در بحث سیگنال فرستادن به موتور های جستجو است که تگ این کار را انجام می دهند. 
...
برای استایل دادن وقتی که موس می رود روی لینک استفاده می شود.
...
برای استایل دادن به لینک هایی که رویشان قبلا کلیک کردیم.
...
برای استایل دادن به لینک ها وقتی رویشان کلیک کردیم.
...
مثال border قابل مقدار دهی می شود.
...
این یعنی برو داخل SELECTORONE دنبال SELECTORTWO باش و آن ها را انتخاب کن.
...
لیست ها به صورت پیش فرض margin و padding دارند.

## استایل دادن به جدول ها

از border-collapse: collapse استفاده می کنیم تا بودر هایی که می شود به هم بچسبند را به هم می چسباند و یکی کنیم.

برای اینکه یکی در میان با سطر های جدول کار کنیم به صورت زیر عمل می کنیم
...
این خصیصه فقط روی تگ های بلاکی با طول و عرض مشخص اعمال می شود.
...
برای سایه دادن به هرچیزی غیر از متن از box-shadow استفاده می کنیم.

برای متن از text-shadow استفاده می کنیم.
...
برای اینکه از یک فونت خاص استفاده کنیم باید 4 فرمت از آن را داشته باشیم که عبارت است از eot, ttf,woff, woff2 است.
...
از خصیصه background-image برای نمایش عکس به عنوان پس زمینه استفاده می کنیم.

که آدرس عکس را توسط url به خصیصه background-image تگ مورد نظر می دهیم.
...
مقدارش مساوی linear-gradient() باشد.

اول جهت را مشخص می کنیم مثلا to right یا to left top

بعدش به ترتیب رنگ اول، رنگ دوم زنگ سوم و....

برای راحتی از سایت [cssgradient.io](https://cssgradient.io/) استفاده کنید.

مقدار radial-gradient هم داریم.
...
وقتی به تگی padding و border می دهیم به طول و عرض آن اضافه می شود.
...
برای همین داخل مرورگر مان سرچ می کنیم [css reset](https://www.google.com/search?q=css+reset) و هر سایتی که خواستیم کد css را می گیریم و داخل یک فایل css که بهتر است اسمش reset.css باشد قرار می دهیم و به صفحه html مان لینک می کنیم.
