...
* [تراز بندی عمودی](#ترازبندی-عمودی)
* [لوگو تب مرورگر](#لوگو-تب-مرورگر)
...
<img src="./outline-in-css.jpg" alt="outline">
...
:anger:اگر opacity را کم کنیم آن تگ و تگ های داخلش کم رنگ می شوند. برای اینکه تگ های داخلش کم رنگ نشوند از background-color: rgba() استفاده می کنیم.
...
:eyes:فونت arial را همه سیستم ها دارند.
...
:small_red_triangle_down:برای ساخت انیمیشن دلخواه از keyframes استفاده می کنیم.
...
:small_red_triangle_down:بعد هرجایی که لازم بود بهش خصیصه animation-name را می دهیم با اسم keyfames مان.

:small_red_triangle_down:با خصیصه animation-duration مدت:alarm_clock: زمان انیمیشن را مشخص می کنیم.

:small_red_triangle_down:با خصیصه animation-iteration-count تعداد تکرار انیمیشن را مشخص می کنیم که اگر برایر infinite قرار دهیم یعنی بی نهایت بار اجرا شود.

:small_red_triangle_down:با خصیصه animation-delay تاخیر انیمیشن را مشخص می کنیم.
...
:small_red_triangle_down:به جای موارد بالا می توان از خصیصه animation استفاده کرد.
...
:small_red_triangle_down:به جای  from و to می توانیم از درصد استفاده کنیم.
...
:zap:بهتر است ابدا و انتهای بخش های مختلف سایت را با کامنت گذاری مشخص کنید:
...
:zap:با inspect element می توانیم کد های html و css را کامل مشاهده کنیم.

:zap:بهتر است ترتیب کدنویسی css به ترتیب تگ ها و در برگیرنده هایشان باشد.

:zap:تو طراحی وقتی چیزی نیست و در حالت خاص مثل hover می خوایم نمایش داده شود بهتر است در حالت عادی آن را قرار دهیم ولی مخفی کنیم تا بهتر نمایش داده شود. مثلا اگر در حالت hover بوردر داریم ولی در حالت عادی نداریم این حالت باعث می شود تا زمانی  که hover می کنیم المان در صفحه جابه جایی شود برای همین در حالت عادی هم بهش بوردر می دهیم ولی هم رنگ با رنگ پس زمینه که معلوم نشود.

:zap:وقتی می خواهید به تگی position: absolute بدهید، بهتر است به تگ پدرش position: relative بدهید تا نسبت به آن جا به جا شود.

:zap:الکی خط کد زیاد نکنید؛ پس کد تکراری تا می توانید نزنید و کد های تکراری را یکی کنید.

:zap:برای دادن چند تا کلاس به یک تگ اسم کلاس ها را با فاصله از هم جدا می کنیم.

```html
<div class="box box-1"></div>
```

:zap:برای راحتی بهتر با alt + tab بین صفحات مختلف سیستم عامل تان جا به جا شوید.

:zap:فونتی که به body می دهید روی input ها اعمال نمی شود و باید به صورت جدا گونه به آن ها هم فونت مدنظرتان را بدهید.

:zap:در مرورگر ها input submit به صورت پیش فرض border دارد.

:zap:برای علامت کپی رایت از `&copy;` استفاده می کنیم.

برای اطلاعات بیشتر می توانید داخل نت سرچ کنید :mag:[html symbol](https://www.google.com/search?q=html+symbol)

:zap:اگر کد ها یادتان می رود عادی است فقط تمرین کنید، پروژه بزنید و سرچ کنید.

:zap:اگر خواهید ارتفاع تگی را 100% کنید علاوه بر دادن height: 100% به آن تگ به html و body هم باید height: 100%; را بدهیم.

:zap:برای متن تایپینگ از type js استفاده می کنیم.

:zap:از کد js قرار اجرا شود اخر فایل قبل از تمام شدن تگ body قرار می دهیم.

:zap:برای کاروسل از کتابخانه slick استفاده می کنیم.

:zap:برای ارجعیت دادن از !important استفاده می کنیم.

:zap:تو آدرس ایمیل به جای @ از [at] و به جای . از [dot] ااستفاده کنید.
...
:zap:بهتر از داخل کد های css کد زیر را قرار قرار دهیم.
...
:zap:با افزونه live server با اعمال تغییرات و بعد از آن ذخیره کردن، صفحه وب به صورت اتوماتیک بروزرسانی می شود.
...
:zap:برای یادگیری بهتر کد ها رو کپی نکنید حداقل خودتان از روش بنویسید.

:zap:خود VS Code هم color picker دارد برای استفاده کافی است روی مربعی که رنگ را نشان می دهد کلیک کنید.

:zap:ساخت منو باز و بسته شونده با js 
...
:bangbang:(به جای ADDRESS آدرس عکس را بهش می دهیم.)
...
:bangbang:(به جای ADDRESS ادرس فایل css را می دهیم.)
...
## ایده یابی
سایت :mag:[dribbble](https://dribbble.com/) برای ایده گرفتن سایت خوبی است.

## فونت آیکن

از فونت ایکن برای قرار دادن ایکن استفاده کنید.

برای فونت ایکن :mag:[font awesome](https://fontawesome.com/) گزینه خوبی است.
...
:beginner:ایکن های free font awesome سه دسته هستند solid, regular, brand.

:beginner:اگر با Fas در font awesome شروع شود یعنی ایکن مان solid است.

:beginner:اگر با Far در font awesome شروع شود یعنی ایکن مان regular است.

:beginner:اگر با Fab در font awesome شروع شود یعنی ایکن مان brand است.

## after & before

:fleur_de_lis:با استفاده از :before و خصیصه content می شود متنی به قبل از یک تگ اضافه کرد.

:fleur_de_lis:با استفاده از :after و خصیصه content می شود متنی به بعد از یک تگ اضافه کرد.
...
:triangular_flag_on_post:و بعد به المان هایی که می خواهیم در attribute class col-N مدنظرمان را قرار می دهیم.

:bangbang:(در متن بالا N عدد مدنظرمان است.)
...
:diamond_shape_with_a_dot_inside:مقدار هر col را برای ریسپانسیو کردن سایت باید مقدار col ها را با @media تغییر داد.

## flexbox

بدون flex box یک container قرار می دهیم سپس به المان های داخل متناسب با آن درصد می دهیم.

:white_check_mark:ویژگی flexbox
...
:white_check_mark:دو نوع flex box داریم
...
:high_brightness:برای ساخت flexbox به container خصیصه display: flex; را می دهیم بعدش مشخص می کنیم عمودی باشد یا افقی.

:high_brightness:خصیصه flex-direction را به container می دهیم و مشخص می کند که flex box عمودی هست یا افقی و مقدار پیش فرض آن row است.
...
:high_brightness:خصیصه justify-content را به container می دهیم و مرتب سازی المان ها در راستای محور اصلی main axis را انجام می دهد.
...
:high_brightness:خصیصه align-items را به container می دهیم و مرتب سازی المان ها در راستای محور عرضی cross axis را انجام می دهد.
...
:high_brightness:خصیصه flex-wrap را به container می دهیم و مشخص می کنیم المان ها در یک خط باشه یا نه.
...
:high_brightness:خصیصه align-content را به container می دهیم و برای جایگذاری المان هایی هست که در چند خط قرار می گیرند. پس این خصیصه با flex-wrap با هم استفاده می شود.
...
:high_brightness:خصیصه align-self را به item هایمان می دهیم و برای تغییر align-item یک المان به صورت اختصاصی است.
...
:high_brightness:خصیصه order را به item های مان می دهیم و ترتیب قرار گیری المان ها را مشخص می کند.

پیش فرض آن صفر است.

عدد کمتر زودتر، عدد بزرگ تر دیرتر و عدد مساوی به ترتیب کد html می آیند.

:high_brightness:خصیصه flex-basis را به item های مان می دهیم و سایز پیش فرض یک المان در راستای main axis است.

اگر flex box در حالت row باشد نقش width را دارد و در حالت column حالت height را ایفا می کند

واحد یادتان نرود.

:high_brightness:خصیصه flex-grow را به item های مان می دهیم و مشخص می کند که یک المان تا چه میزان می تواند بزرگ شود و فضای خالی موجود چطور بین المان ها تقسیم شود.

این در حالی است که فضای خالی داریم.

عدد هایی که می نویسیم نسبی هستند و پیش فرض آن صفر است.

:high_brightness:خصیصه flex-shrink را به item های مان می دهیم و مشخص می کند زمانی که فضای کافی در دربرگیرنده وجود ندارد یک المان تا چه میزان می تواند کوچک شود.

عدد هایی که می نویسیم نسبی هستند و پیش فرض آن یک است.

:high_brightness:خصیصه flex را به item های مان می دهیم و برای اختصار و کوتاه کردن خط کد است.
...
:mag:[منبع برای یادگیری بیشتر](https://css-tricks.com/snippets/css/a-guide-to-flexbox/)
